...
def run_background_notification():
        while True:
            notif.notif_loop()
background_thread = threading.Thread(target=run_background_notification)
...
def register():
    ...
    def confirm_register():
        ...
        if not saving_password==compare_password:
            error.config(text="Error: Password does not match",foreground="red")
        else:
            c.execute("SELECT COUNT(*) from login_info WHERE Saved_username='"+saving_username+"'")
            result= c.fetchone()
            if int(result[0]) > 0:
                error.config(text="Error: This Username already exist.\nPlease enter another one.",foreground="red")
            else:    
                error.config(text="New user added.",foreground="green")
...
def mainloop():
    def login():
        ...
        if int(userresult[0]) > 0:
            ...
            if int(passwordresult[0]) > 0:
                global CurrentUser
                CurrentUser=str(username_enter)
                root.destroy()
                import CRUD as crud
                crud.CRUD_display()
            else:    
                error.config(text="Incorrect password",foreground="red")
        else:    
    ...
    ttk.Checkbutton(mainframe,text='Show password',command=showpass,variable=ONorOFF,onvalue=1, offvalue=0).grid(column=3,row=3,sticky=W)
    ...
    error.grid(column=1,row=7,sticky=W,columnspan=20)

    ttk.Button(mainframe, text="Login", command=login).grid(column=3,row=5,sticky=E)

    ttk.Button(mainframe, text="Register", command=register).grid(column=1,row=5,columnspan=20,sticky=W)

    ttk.Button(mainframe, text="Forgot password", command=forgot_password).grid(column=1,row=6,columnspan=20,sticky=W)

    appimage = Image.open("C:/Users/justi/OneDrive - student.newinti.edu.my/ALL PROJECT/tdl.png")
    appimage = ImageTk.PhotoImage(appimage.resize((100, 30)))
    ttk.Label(mainframe, image=appimage).grid(column=1,columnspan=200,row=1,sticky=W)
...
def forgot_password():
    ...
    def resetpass():
        ...
        if not saving_new_password==compare_password:
            ...
            result= c.fetchone()
            if int(result[0]) > 0:
                error.config(text="Password updated.",foreground="green")
